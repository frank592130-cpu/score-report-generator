...
def sc(ws, row, col, value, bold=False, size=10, fill=None, border=None, wrap_text=False):
...
def has_allowed_upload_extension(filename):
    return any(filename.lower().endswith(extension) for extension in ALLOWED_UPLOAD_EXTENSIONS)
...
def build_excel(students_data, exam_lines, ths):
    ...
    if n_total > 0:
        ...
        for row in range(avg_start_row, final_row + 1):
...
def upload_read():
    ...
    if not has_allowed_upload_extension(file.filename):
        return jsonify({"students": [], "error": "Only .xlsx and .xlsm files are supported."}), 400
