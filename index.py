...
def read_students_initial(file_stream):
    """
    支援雙格式讀取邏輯：
    1. 讀卡機格式 (姓名在 O 欄/索引 14，選擇分數在 H 欄/索引 7)
    2. A卷格式 (姓名在 A 欄/索引 0，分數在 B 欄/索引 1)
    """
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # 判斷是否為原本的讀卡機格式 (檢查第 O 欄是否有有效名稱)
            is_reader_format = False
            if len(row) >= 15 and row[14] is not None:
                name_o = str(row[14]).strip()
                if name_o not in ["", "預設標準答案", "None"]:
                    is_reader_format = True
            
            if is_reader_format:
                # 執行原本的讀卡機解讀邏輯
                name = str(row[14]).strip()
                try:
                ...
                # 執行全新的 A 卷解讀邏輯：A欄 (索引 0) 姓名，B欄 (索引 1) 分數
                if len(row) >= 1:
                    name_a = str(row[0]).strip() if row[0] is not None else ""
                    if name_a in ["", "姓名", "預設標準答案", "None"]:
                        continue
                    try:
                        x_val = float(row[1]) if len(row) > 1 and row[1] is not None else 0.0
                        students.append({"name": name_a, "x": x_val})
                    except (ValueError, TypeError):
                        continue
        ...
        <div class="glass-card rounded-2xl p-8 space-y-6">
            <form id="main-form" action="/generate" method="post" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="exam_name" placeholder="例如：115學年度 第一次模擬考" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div id="dropzone" class="border-2 border-dashed border-slate-800 hover:border-indigo-500 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer group transition-colors bg-slate-950/20">
                        ...
                        <div id="upload-content" class="text-center">
                            ...
                            <p class="text-xs text-slate-400 font-semibold">點擊或拖曳檔案至此處</p>
                            <p class="text-[10px] text-slate-500 mt-1" id="file-status">支援讀卡機檔案 或 A卷 (A欄姓名/B欄分數)</p>
                        </div>
                    </div>
            ...
            studentsData.forEach((s, idx) => {
                ...
                row.innerHTML = `
                    <div class="flex-1 flex items-center justify-between">
                        ...
                        <span class="text-[10px] text-slate-500">選擇: <b class="text-indigo-300">${s.x}</b> 分/題</span>
                    </div>
...
def analyze_full():
    ...
    counts = {"A++": 0, "A+": 0, "A": 0, "B++": 0}
    sum_sel, sum_nonsel, sum_total = 0, 0, 0
