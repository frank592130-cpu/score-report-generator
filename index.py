    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            try:
                ...
                students.append({"name": name, "x": x_val, "y": 0, "is_leave": False})
...
def build_excel(students_data, exam_lines, ths):
    ...
    # 嚴格依照要求設定欄寬
    # ════════════════════════════════
    ws.column_dimensions['A'].width = 7.25
    ws.column_dimensions['E'].width = 7.25
    ws.column_dimensions['I'].width = 7.25

    ws.column_dimensions['B'].width = 4.50
    ws.column_dimensions['C'].width = 4.50
    ws.column_dimensions['F'].width = 4.50
    ws.column_dimensions['G'].width = 4.50

    ws.column_dimensions['D'].width = 5.75
    ws.column_dimensions['H'].width = 5.75

    ws.column_dimensions['J'].width = 8.88
    ws.column_dimensions['K'].width = 8.88
    ws.column_dimensions['L'].width = 8.88
    ...
    if n_total > 0:
        ...
        # 增加列高以配合 16 號字體
        ws.row_dimensions[r_avg_start].height = 24
...
<body class="p-8 flex justify-center">
    <div class="max-w-4xl w-full space-y-8">
        <header class="text-center"><h1 class="text-4xl font-extrabold bg-gradient-to-r from-indigo-400 to-pink-400 bg-clip-text text-transparent">TIME SAVER</h1></header>
        <div class="glass-card rounded-2xl p-8 space-y-6">
            <form id="main-form" action="/generate" method="post" class="space-y-6">
                ...
                <div>
                    <label class="text-xs font-bold text-slate-500 mb-2 block uppercase">考試名稱</label>
                    <input type="text" name="exam_name" placeholder="例如：國三 金安模擬考 第一回" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div id="dropzone" class="border-2 border-dashed border-slate-800 rounded-xl p-8 text-center hover:border-indigo-500 cursor-pointer">
                        ...
                        <div class="text-2xl mb-1">📁</div><p class="text-xs text-slate-500">讀取 XLSX 檔案</p>
                    </div>
                ...
                <div id="success-bar" class="hidden bg-indigo-500/10 border border-indigo-500/20 py-3 px-4 rounded-lg flex justify-between items-center">
                    <span class="text-xs text-indigo-300">✨ 載入成功：<span id="st-count">0</span> 位學生</span>
                </div>
                <div id="students-scores-container" class="hidden space-y-4">
                    <div id="students-list" class="max-h-64 overflow-y-auto space-y-2 custom-scrollbar"></div>
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] uppercase font-bold text-indigo-400">名單順序</label>
                    <textarea id="ordered_names" name="ordered_names" rows="4" class="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="每行一個姓名..."></textarea>
                </div>
                <div class="flex gap-4 pt-4">
                    <button type="submit" id="submit-btn" class="btn-main flex-[2] py-4 rounded-xl font-bold opacity-50 cursor-not-allowed" disabled>下載正式報表</button>
                    <button type="button" id="copy-btn" onclick="downloadCopyList()" class="flex-1 bg-slate-800 py-4 rounded-xl font-bold opacity-50 cursor-not-allowed" disabled>APP 貼上表</button>
                </div>
            </form>
        </div>
    </div>
    <script>
        ...
        fileInput.onchange = async () => {
            ...
            if (d.students) {
                d.students.forEach(ns => { if(!studentsData.some(s => s.name === ns.name)) studentsData.push({...ns, y: 0}); });
                refreshUI();
            }
        };
        function addManualStudent() {
            const n = document.getElementById('manual-name'), x = document.getElementById('manual-x');
            if (n.value) { studentsData.push({name: n.value, x: parseFloat(x.value)||0, y:0, is_leave:false}); n.value=''; x.value=''; refreshUI(); }
        }
        function refreshUI() {
            const has = studentsData.length > 0;
            document.getElementById('st-count').innerText = studentsData.length;
            document.getElementById('success-bar').classList.toggle('hidden', !has);
            document.getElementById('submit-btn').disabled = !has;
            document.getElementById('copy-btn').disabled = !has;
            document.getElementById('submit-btn').classList.toggle('opacity-50', !has);
            document.getElementById('copy-btn').classList.toggle('opacity-50', !has);
            renderList(); updateDash();
        }
        function renderList() {
            ...
            studentsData.forEach((s, i) => {
                ...
                div.innerHTML = `<span class="text-sm">${s.name} (選擇: ${s.x})</span><input type="number" step="0.5" value="${s.y}" class="w-16 bg-slate-900 border border-slate-700 text-center rounded" onchange="studentsData[${i}].y=parseFloat(this.value);updateDash();">`;
                container.appendChild(div);
            });
            document.getElementById('students-scores-container').classList.remove('hidden');
        }
        function updateDash() { document.getElementById('students-json').value = JSON.stringify(studentsData); }
        function downloadCopyList() {
            const form = document.getElementById('main-form');
            const old = form.action; form.action = '/generate_copy_list'; form.submit();
            setTimeout(() => form.action = old, 500);
        }
    </script>
</body>
</html>'''
...
def generate_copy_list():
    students = json.loads(request.form.get('students_json', '[]'))
    names = request.form.get('ordered_names', '').split('\n')
    student_map = {s['name']: s for s in students}
    wb = openpyxl.Workbook()
    ws = wb.active
    for i, n in enumerate([x.strip() for x in names if x.strip()], 1):
        ws.cell(row=i, column=1, value=n)
        if n in student_map:
            s = student_map[n]
            total = (float(s['x'])/25*85) + (float(s['y'])/6*15)
            ws.cell(row=i, column=2, value=round(total, 2))
    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    return send_file(buf, as_attachment=True, download_name="APP_Import.xlsx")
